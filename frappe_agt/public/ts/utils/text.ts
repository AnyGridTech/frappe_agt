frappe.provide('agt.text');

agt.text.normalize = function(text: string): string {
  return (text || '')
    .toLowerCase()
    .normalize('NFD')
...
agt.text.is_valid = function(v: any) {
  return v !== null && v !== undefined && (typeof v === 'string' ? v.trim() !== '' : true);
};

agt.text.to_snake_case = function(str: string) {
...
agt.text.to_pascal_case_spaced = function(str: string) {
  return str
